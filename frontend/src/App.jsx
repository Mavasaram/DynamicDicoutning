        ...
        <AnimatePresence mode="wait">
          {mode === 'landing' && (
            <motion.div
              ...
              className="flex-1 min-h-0 overflow-y-auto flex flex-col items-center gap-6 px-4 py-10"
            >
              <div className="text-center max-w-2xl">
                <div className="text-5xl mb-4">🤖</div>
                <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2">
                  Payable Agent <span className="text-gold-400">AI</span>
                  <span className="block text-base sm:text-lg font-normal text-slate-400 mt-1">
              ...
              <div className="w-full max-w-sm space-y-1.5">
                {STEPS.map((s, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-navy-800/60 border border-navy-700 text-slate-300 text-sm"
                  >
              ...
              {error && (
                <div className="bg-red-900/30 border border-red-700 rounded-lg px-4 py-3 text-red-300 text-sm max-w-md text-center">
                  {error}
                </div>
              )}

              <div className="flex flex-col sm:flex-row gap-3 items-center">
                <button onClick={runSample} className="btn-primary flex items-center gap-2">
                  <span>⚡</span> Load Sample Data &amp; Analyse
                </button>
                <span className="text-slate-600 text-sm">or upload your own files below</span>
              </div>
          ...
          {mode === 'processing' && (
            ...
            >
              <ProcessingAnimation steps={PROCESSING_MSGS} currentStep={processingStep} />
            </motion.div>
          )}
          ...
          {mode === 'results' && data && (
            ...
            >
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
                {data.has_discrepancy && (
                  <motion.div
                    initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                    className="bg-amber-900/30 border border-amber-600/50 rounded-xl px-4 py-3 flex items-start gap-3"
                  >
                    <span className="text-xl shrink-0">⚠️</span>
                    <div>
                      <p className="font-semibold text-amber-400 text-sm">Payment Terms Discrepancy Detected</p>
                      {data.discrepancy_details.map((d, i) => (
                        <p key={i} className="text-amber-300/80 text-xs sm:text-sm mt-0.5">{d}</p>
                      ))}
                    </div>
                  </motion.div>
                )}
