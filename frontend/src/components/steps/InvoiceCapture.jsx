  ...
  return (
    <section className="card">
      <div className="section-title">
        <span className="w-7 h-7 bg-gold-500/20 rounded-full flex items-center justify-center text-gold-400 font-bold text-sm shrink-0">1</span>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {/* Invoice fields */}
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Invoice Fields</p>
            ...
            <table className="w-full min-w-[260px]">
              ...
              <tbody>
                {fields.map(({ label, value }, i) => (
                  <motion.tr key={label} className="tbl-row"
                    initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.06 }}>
                    <td className="tbl-cell text-slate-400 font-medium whitespace-nowrap">{label}</td>
              ...
              <tbody>
                {invoice.line_items.map((item, i) => (
                  <tr key={i} className="tbl-row">
                    <td className="tbl-cell mono text-blue-400 whitespace-nowrap">{item.sku}</td>
                    <td className="tbl-cell text-slate-300">{item.description}</td>
                    <td className="tbl-cell text-right">{item.quantity}</td>
                    <td className="tbl-cell text-right mono whitespace-nowrap">{fmt(item.unit_price)}</td>
                    <td className="tbl-cell text-right mono font-semibold text-white whitespace-nowrap">{fmt(item.extended_amount)}</td>
                  </tr>
                ))}
                <tr className="bg-navy-800">
                  <td colSpan={4} className="tbl-cell text-right font-semibold text-slate-400">Total</td>
                  <td className="tbl-cell text-right mono font-bold text-gold-400 whitespace-nowrap">{fmt(invoice.invoice_amount)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {excelInvoices && excelInvoices.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Invoice Register (Excel)</p>
          <div className="overflow-x-auto rounded-lg border border-navy-700">
            <table className="w-full min-w-[420px]">
              <thead>
                <tr className="tbl-head">
                  ...
                  <th className="tbl-cell text-left hidden sm:table-cell">Date</th>
                  <th className="tbl-cell text-right">Amount</th>
                  <th className="tbl-cell text-left hidden sm:table-cell">Terms</th>
              ...
              <tbody>
                {excelInvoices.map((row, i) => (
                  <tr key={i} className={`tbl-row ${row.invoice_number === invoice.invoice_number ? 'bg-gold-500/5' : ''}`}>
                    <td className="tbl-cell mono text-blue-400 font-medium whitespace-nowrap">{row.invoice_number}</td>
                    <td className="tbl-cell text-slate-300">{row.vendor}</td>
                    <td className="tbl-cell text-slate-400 hidden sm:table-cell">{row.invoice_date}</td>
                    <td className="tbl-cell text-right mono whitespace-nowrap">{fmt(row.amount)}</td>
                    <td className="tbl-cell text-slate-400 hidden sm:table-cell">{row.payment_terms}</td>
